...
def predict_next_day():
    ...
    #load the model
    saved_model=  pk.load(open('D:\ProgramData\Anaconda3\envs\SMP_Env\SMP_proj\Model\saved_data\saved_AAPL.sav', 'rb'))
...
def plot_predicrtions_next_day():
        
        day_new=np.arange(1,day_step+1)
        day_pred=np.arange(day_step+1,day_step+next_days+1)
        plt.plot(day_new,df[len(df)-day_step:])
        plt.plot(day_pred,output)
        plt.plot(day_pred[-1],predict_next_day(),'ro')
